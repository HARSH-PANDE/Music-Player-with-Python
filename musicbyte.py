...
windows = tk.Tk()
windows.title('OldSchool Mp3 Player')
...
def updatetitle(title):
    ...
    global filefound
    if filefound != True:
        title = f'OldSchool Mp3 Player - Add songs to playlist first! ' + title
        root.title(title)
        root.update()
        return
    title = f'OldSchool Mp3 Player - Playing:   ' + title
    windows.title(title)
    windows.update()
...
def play(check):
    ...
    if song != checksong:
        ...
        if pausedornot:
            tl = tl.replace('       ', '')
            t = f'OldSchool Mp3 Player - Playing:   ' + tl
...
def aboutwindow():
    ...
    height = 570
    win = tk.Toplevel()
    win.wm_title("OldSchool Mp3 Player - About")
    screen_width = windows.winfo_screenwidth()
    screen_height = windows.winfo_screenheight()
    ...
    lbl1 = Label(infoframe, text='OldSchool Mp3 Player', font=('AdobeClean-Bold', 13))
    ...
    about = Label(aboutframe, text='OldSchool Mp3 Player is a Stylish, Powerful and  Fast Music Player  with  elegant design. \n'\
    ...
    features = Label(aboutframe, text='Currently OldSchool Mp3 Player fully Supports (Features): \n\n'\
    ...
    footerlbl = Label(footer, text='OldSchool Mp3 Player is a Music  Player  for  Windows. Send  me  the feedbacks,\n'\
                                    'bug-reports and suggestions about OldSchool Mp3 Player to:', justify=LEFT)
    footerlbl.place(x=0,y=0)
    emaillbl = Label(footer, text='thesahil18@gmail.com',fg="blue", cursor="hand2")
...
def licenses():
    ...
    height = 480
    win = tk.Toplevel()
    win.wm_title("OldSchool Mp3 Player - View Licenses")
    screen_width = windows.winfo_screenwidth()
    screen_height = windows.winfo_screenheight()
    ...
    vlbl = Label(infoframe, text='Version 1.2.1')
    vlbl.place(x=160,y=30)
    mit = Label(win, text='ABCD LICENCE')
    mit.place(x=208,y=80)
    copyryt = Label(win, text='Copyright (c) 2021 SAHIL KUMAR')
    copyryt.place(x=150,y=105)

    bottomfrm = Frame(win, height=290, width=420)
    bottomfrm.place(x=45, y=130)
    textlbl = Label(bottomfrm, text='lorem34', justify=LEFT)
...
headlbl = Label(head, image=pic, bg=leftFrameColor) #font=('CoolveticaRg-Regular', 25) text='OldSchool Mp3 Player',
